...
- **docs/** —— 仅保留当前工作文档：`prd/PRD-20260908/`（对齐基准，md 为唯一口径）、`PRD-review/`（改动与对齐记录）及各审查/决策目录；已归档的历史文档不随仓库分发（2026-09-10 起统一存负责人本机 `Archive/`，仓库侧可经 git 历史找回）。
...
├── docs/
│   ├── prd/PRD-20260908/     # PRD 对齐基准（各模块 md 为唯一口径 + 必填选填一览表；原型已退场）
...
- PRD 对齐基准：`docs/prd/PRD-20260908/数字员工管理端PRD/`——各模块 prd md（**唯一口径**；交互原型已退场，旧原型与截图仅历史参考）+《各模块必填选填字段一览表.md》
