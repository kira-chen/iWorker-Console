...
describe('positionApplicationsMock —— 待分配申请（新流程：举手申请不指定岗位，管理端统一分配）', () => {
  it('种子 3 条：chenyu(3) / sun.xin(6) 待分配，li.na(2) 历史已分配；徽标计数 = 2', async () => {
    expect(getPendingApplicationByUserId(3)).toMatchObject({ id: 701, status: 'PENDING' })
    expect(getPendingApplicationByUserId(6)).toMatchObject({ id: 702, status: 'PENDING' })
    expect(getPendingApplicationByUserId(2)).toBeNull() // 703 已 ASSIGNED，不算待分配
    expect((await countPendingApplications()).count).toBe(2)
  })

  it('getPendingApplicationByUserId：userId 数字/字符串同义；无申请的用户返回 null', () => {
    expect(getPendingApplicationByUserId('3')).toMatchObject({ id: 701 })
  ...
  it('联查到分配列表：有待分配申请的行带 hasPendingRequest/pendingRequestId/pendingRequestAt；hasPendingRequest=true 只筛出这些行（md §一.18 / §二「待分配」标签）', async () => {
    expect(await assignmentOf(3)).toMatchObject({ hasPendingRequest: true, pendingRequestId: 701, pendingRequestAt: '2026-08-28 10:32' })
    expect(await assignmentOf(2)).toMatchObject({ hasPendingRequest: false, pendingRequestId: null })
    const { list } = await listPositionAssignments({ hasPendingRequest: true })
    expect(list.map((r) => r.userId).sort()).toEqual([3, 6])
  })
  ...
  it('markApplicationAssigned：PENDING → ASSIGNED 留痕（assignedAt/assignedBy），徽标递减，分配列表上「待分配」标签消失', async () => {
    ...
    expect(getPendingApplicationByUserId(3)).toBeNull()
    expect((await countPendingApplications()).count).toBe(1)
    expect(await assignmentOf(3)).toMatchObject({ hasPendingRequest: false })
  })
...
describe('positionApplicationsMock · 持久化读回（mockPersist v3；写点 markApplicationAssigned → 刷新后仍在）', () => {
  ...
  it('markApplicationAssigned(701) 落盘（v=3）→ 重新 import 模块 → 701 仍是已分配、待分配数为 1', async () => {
    ...
    expect(fresh.getPendingApplicationByUserId(3)).toBeNull()
