 ...
 * - 未发布 → 【发布】（先跑 md §9.1 七项完整性校验，Q3 不弹确认窗，通过则直接开版本管理侧栏）
 *   +【删除】（领用护栏 + 确认文案照新 md）；
...
// 2026-09-09 PRD 复核·G2（A1）：【发布】门改跑 md §9.1 七项完整性校验，需读岗位详情 + 自动化任务条数。
...
// md §9.1 七项齐备的岗位详情（A1 发布门入参）：名称/描述/领用页文案/示例问题 3 条/SOP/Agent 与技能/自动化任务
const FULL_DETAIL = {
  name: '可发布草稿岗',
  description: '描述',
...
describe('AdminPositions 操作列（原型 positionActions 口径）', () => {
  ...
  // 七项完整性校验（computeCompletenessMissing），缺项 toast「请先填写：…」并跳详情页对应页签。
  // 2026-09-09 负责人拍板：列表页与详情页两个【发布】入口行为一致，七项齐备后统一开
  // 「发布前检查弹窗」（md §3.3/§9.2）。原断言「直接开版本管理侧栏」是 Q3 旧口径（原型作
  // 基准时的处理），原型已退场故推翻。【版本管理】按钮仍走 VersionDrawer，见 ②e。
  it('②b 发布（未发布行）：七项齐备 → 开发布前检查弹窗，不开版本侧栏、不弹 confirm', async () => {
    await mount()
    ...
    expect(dlg.getAttribute('data-passed')).toBe('true') // 七项齐备 → 可发布
