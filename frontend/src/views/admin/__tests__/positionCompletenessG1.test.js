 ...
 *
 * 本组钉三件事：
 *  ① 【发布岗位】七项任一缺失 → 不开发布弹窗、toast「请先填写：…」、自动切到第一个缺失项所在页签；
 *  ② 【保存】执行同一套七项校验但**不阻断**：保存照常完成，顶部提示条列出未完成项（可关、可点跳页签）；
 *  ③ 七项齐备时保存不出提示条、发布放行到发布前检查弹窗。
 *
...
const store = {
  ...
  get checkInput() {
    return {
      name: this.basic.name, description: this.basic.description, claimDescriptions: this.basic.claimDescriptions, positionSop: this.basic.positionSop,
...
// 自动化任务条数：md §9.1 第 7 条的入参，详情页挂载即独立预取（不依赖是否访问过该页签）
...
describe('A1 · 【发布岗位】按 md §9.1 七项硬阻断', () => {
  it('七项齐备 → 放行到发布前检查弹窗，无 toast', async () => {
    await mount()
  ...
  it('第 3 项：领用页文案为空 → 阻断、不开弹窗、toast 点名、定位人格页签（2026-09-21 负责人拍板必填）', async () => {
  ...
  it('第 6 项：无 Agent / Agent 无技能 → 阻断并定位「Agent 与技能」页签', async () => {
  ...
  it('第 7 项：自动化任务 0 条 → 阻断并定位「自动化任务」页签', async () => {
