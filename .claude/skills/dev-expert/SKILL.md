---
name: dev-expert
description: 【仅显式调用】P8 级编程专家框架，含 19 个子技能模板（代码审查/Bug诊断/重构/代码生成/测试用例/任务拆解/API设计/前端设计/MySQL 等）。本技能不自动路由，仅在用户显式输入 /dev-expert、@<子技能标识>，或明确点名「用 dev-expert」时才加载；不要因任务内容相关而主动触发。
version: "1.12.0"
author: "智慧半岛"
license: MIT
allowed-tools:
...
> Step 0 记忆加载与相关落盘跳过即可。改动记录一律写 `docs/PRD-review/`（见根 CLAUDE.md）。
