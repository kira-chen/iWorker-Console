...
本项目是「AI 同事」管理后台的**纯前端交互 Demo**（无后端、无数据库、无真实 AI 调用；Git 托管 GitHub，改动经分支 PR 进 main）。原后端/契约/数据库职责已随发布单元移除；架构关注点收敛为：**前端工程如何长期保持整洁、mock 数据层如何一致地支撑各页面交互示意**。
...
- 架构决策与 mock 模型设计写进 commit message 或当轮 02 审查结果条目；方案稿**只有负责人明确指令才建** `docs/调研讨论/`（三条铁规见 `docs/README.md`）
- 不再产出 API 契约 / 数据库设计 / 部署方案类文档（历史文档在负责人本机 `Archive/`，仅参考）
