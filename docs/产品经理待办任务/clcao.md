...
| 4 | 未处理 | 陈森亮（2026-09-23） | **【严重】运行规格：删岗位后该规格永久删不掉 + 三处小缺陷（04 运行）。** ①**死锁**（与浦月 #23① 同一处缺口，那边记的是岗位侧、这边是运行规格侧，**建议两边一起修**）：`positionMock.js:327` 删岗位不清 runtimeSpec，而 `runtimeSpecMock.js:108` 显示用 `positionIds.map(id => positionMap.get(id)).filter(Boolean)` 把已删岗位**静默丢弃**、`:252` 删除守卫却按 `positionIds.length` **原始长度**判 → 列表显示「0 个岗位」、点删报「已配置给 1 个岗位」，而编辑器下拉又选不出已删岗位，**UI 无任何解除路径，该规格永久锁死**。种子 `runtimeSpecMock.js:21,23` 就绑着 401/402，删掉任一即触发。②`runtimeSpecMock.js:15` `directUsers` 以 **username** 存关系而非 userId，`adminUserMock.js:200-210` 删用户不级联 → 同款死锁（规格显示 0 个个人配置、删除守卫仍按 `directUsers.length` 拦）。③`:122-126` `occupyPositions` 把岗位从原规格摘掉时不更新其 `updatedAt`（md §三.2 要求）。④`AdminRuntimeSpecs.vue:30` 传 `pageSize: 10` 误用了 `useAdminList` 的 `fixedPageSize`（`composables/useAdminList.js:49` 注释写明该参数「仅单测用」），是全站唯一一个传固定值的列表页 → 切换每页条数不重拉。⑤`RuntimeSpecUserDialog.vue:46-47` 配置范围搜索只匹配 `[username, displayName, currentSpecName]`，不含岗位名，而 mock 侧 `runtimeSpecMock.js:168` 的 `listRuntimeSpecUsers` 关键字**是**含 `positionName` 的（md §三.3.4 要求匹配岗位名）→ 弹窗本地过滤与数据层口径不一致。期望：①②按 userId/存在性过滤并与删除守卫同源；③补 updatedAt；④去掉 fixedPageSize；⑤搜索补 positionName。 |
