...
- **性质**：展示"读"或"写"标签。
- **引用情况**：岗位私有有引用时展示"N 个岗位引用"，市场连接器有引用时展示"N 个技能引用"，点击弹出引用清单（展示岗位名或技能名）；无引用展示"暂无引用"；通用连接器显示"—"。引用清单弹窗标题固定为"被技能引用"，底部仅【关闭】按钮。
...
- **连接器类型**：必选单选"岗位私有 / 市场连接器 / 通用连接器"，下方提示"连接器类型创建后不可更改"。新建时必须选择，编辑时置灰不可修改。
- **所属岗位**：仅当连接器类型为"岗位私有"时展示。可选择已发布的岗位进行绑定；未绑定时显示"未绑定岗位"。绑定后该连接器仅在对应岗位下可用。
