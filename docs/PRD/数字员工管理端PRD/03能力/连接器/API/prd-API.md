...
- **页面入口**：侧边栏"03 能力 / 连接器 / API"。页面标题"连接器"，说明"平台全部连接器 —— 通用连接器 / 岗位私有 / 市场连接器 统一管理"；顶部保留 MCP、API、业务系统三个页签，当前选中 API。
- **搜索框**：占位提示为"搜索 API 名称或描述"，支持按 API 名称或描述模糊搜索。
...
- 搜索和状态筛选可组合使用；点击【查询】后按当前条件刷新；清空搜索框内容时列表自动刷新。
...
- **API**：展示图标、API 名称，名称下方展示描述（缩略，悬停查看完整内容）。
- **状态**：独立列展示发布状态标签【未发布】【审核中】【已发布】，紧跟 API 列之后。
...
- **验证**：展示连接状态（未探测 / 连接正常 / 连接异常）、最近验证时间（格式为"MM-DD HH:mm"）和【↻ 重新验证】入口。
...
**分页规则**：本页按**服务提供系统**分页，不按 API 分页——即一页展示 N 个服务提供系统，每个系统下的 API 全部展示，不会出现同一系统的 API 被拆到两页的情况。底部分页器展示"共 N 个 · 每页 X 个"（计数单位为服务提供系统的"个"，区别于其余列表页的"条"），每页个数按页面可用高度动态计算，与其余列表页共用同一套分页控件。搜索或状态筛选后，只有命中 API 的系统参与分页计数。
...
- 验证状态：未探测、连接正常、连接异常（验证进行中沿用发起前状态，按钮显示进行中态）。
...
- **名称**：必填，最多 64 字符，API 的平台展示名称；占位"如 报销查询 API"。
- **图标**：必填。
...
- **示例问题**：必填，固定 3 条输入行，位于基本信息卡片内；用于帮助用户理解如何使用该工具。区块标题右侧展示一枚【AI 生成】按钮（一次生成全部 3 条），使用常规字重，不使用加粗样式，点击后调用模型根据 API 名称和描述自动生成示例问题，填入输入行；重复点击可重新生成，覆盖当前内容。单条示例问题最多 300 字符。

### 3. 鉴权配置

- **鉴权类型**：可选择"不鉴权 / API KEY / Bearer Token"，默认不鉴权。
- 凭证会静态附加到每次请求。
- 选择"不鉴权"时不展示凭证字段。
...
| 列 | 必填 | 规则 |
|---|---:|---|
| 参数名 | 是 | 最多 128 字符；如 `X-Api-Key`。 |
| 描述 | 否 | 最多 200 字符；参数用途说明，标注"客户端可见"。 |
| 客户端填写 | 否 | 勾选后该参数由客户端用户在使用时填写，平台值输入框禁用。 |
| 位置 | 是 | Query、Body、Header、Path 之一。 |
| 参数值 | 条件必填 | 未勾选"客户端填写"时必填；按敏感信息处理，以密码形式输入；查看态不明文展示，保存后遮罩。 |
...
| 字段 | 必填 | 规则 |
|---|---:|---|
| Bearer Token | 是 | 不含 `Bearer` 前缀；以密码形式输入，保存后遮罩；查看态不明文展示。 |

### 4. 请求配置

- **请求方式**：必选 GET、POST、PUT、DELETE、PATCH 之一，默认 GET。
- **API 地址**：必填，须为合法的 HTTP 或 HTTPS URL，最多 500 字符。
- **所属服务提供系统**：必填，下拉选择已有服务提供系统，用于 API 分组。
- **读 / 写标记**：必填，标记该 API 属于查询类（读）还是操作类（写），技能引用后据此派生「操作类 / 查询类」标签。
- **启用 / 停用状态**：必填，默认启用；停用后技能不再可引用该 API。
...
- 点击【＋ 添加字段】新增一级字段；字段行包含：字段名、字段类型、请求方法、是否必填、默认值、字段说明和删除操作。
...
- **请求方法**：下拉选择该参数的传递位置，可选 Query / Path / Header / Body；仅请求参数展示该列。
- **是否必填**：字段级复选项，勾选后表示运行时必须提供或返回该字段。
- **默认值**：选填，调用方未传该参数时使用的取值；仅请求参数展示该列。
...
- 选择 API KEY 时，至少保留一行有效参数，且每行参数名不能为空；未勾选"客户端填写"的行参数值不能为空。
- 选择 Bearer Token 时，Bearer Token 不能为空。
