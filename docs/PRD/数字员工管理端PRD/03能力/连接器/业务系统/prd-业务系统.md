...
- **示例问题**：必填，固定 3 条输入行，位于基本信息卡片内；用于帮助用户理解如何使用该连接器。区块标题右侧展示一枚【AI 生成】按钮（一次生成全部 3 条），使用常规字重，不使用加粗样式，点击后调用模型根据系统名称和描述自动生成示例问题，填入输入行；重复点击可重新生成，覆盖当前内容。单条示例问题最多 60 字符。
