...
- 输入或清除搜索内容后，点击【查询】按钮刷新结果，并回到第 1 页。
- 切换或清空连接器类型、状态筛选后，页面按照当前条件刷新，同时回到第 1 页。
...
- **服务**：同一列主要展示 MCP 名称，名称使用加粗字重，辅助展示服务描述；已经设置图标时在名称前展示图标。描述未填写时显示”—“，内容过长时缩略展示，鼠标悬停可查看完整内容。
- **状态**：独立列展示发布状态标签【未发布】【审核中】或【已发布】，紧跟服务列之后。
- **连接器类型**：展示”岗位私有、市场连接器、通用连接器”之一，普通文本样式。
- **传输方式**：作为独立列展示”stdio”、”streamable-http”或”sse”；无内容时显示”—“。
- **工具数**：有工具时展示具体数量；无工具时显示”—“，鼠标悬停提示”尚未拉取到工具，请在编辑器内「拉取工具」”。
- **引用情况**：岗位私有有引用时展示”N 个岗位引用”（N 为引用了该连接器的岗位数，引用关系在岗位侧产生，连接器侧不可编辑），市场连接器有引用时展示”N 个技能引用”，点击弹出引用清单（展示岗位名或技能名）；无引用展示”暂无引用”；通用连接器显示”—“。引用清单弹窗标题按引用方类型展示：岗位私有为”被岗位引用”（列岗位名），市场连接器为”被技能引用”（列技能名）；底部仅【关闭】按钮。
...
- 未发布的 MCP 显示【发布】（已驳回、已停用统一归入「未发布」状态，见§四）。
...
- 确认窗口内容：”将把「{服务名称}」下全部 {N} 个工具作为一个整体提交审核，审核通过后才对平台各项服务开放。”
- 确认按钮显示【提交审核】。
...
- 确认窗口按待审类型说明恢复结果：待审发布撤回后回到“未发布”，后续需重新发布并再次审核；待审停用撤回后保持“已发布”，继续对客户端提供服务。
- 确认按钮显示【撤回】。
- 撤回成功后提示“已撤回”，状态按上述类型恢复。
...
- 列表支持分页，每页条数按窗口高度动态计算，与平台其余列表页采用同一套分页策略。
- 底部展示总条数、每页条数和分页器。
- 分页器展示上一页、页码、下一页和数据总数。
- 翻页后加载所选页的数据。
...
- 输入区域默认展示 MCP 配置示例。
- 页面支持识别 stdio 配置中的 Command、Arguments、Environment，以及 http 配置中的地址。配置里声明类型为 sse 的，按“sse”导入；声明为 http、streamable-http 或仅有地址未声明类型的，按“streamable-http”导入。不会把 sse 悄悄转成 streamable-http。
...
- **连接器类型**：必选单选”岗位私有 / 市场连接器 / 通用连接器”，下方提示”连接器类型创建后不可更改”。新建时必须选择，编辑时置灰不可修改。**配置页只选择连接器类型，不绑定具体岗位**：选择”岗位私有”后表单不出现”所属岗位”等岗位绑定项；岗位私有连接器不归属于某一个岗位，由岗位侧”连接器”页签引用，同一个连接器可被多个岗位重复引用（见岗位 PRD）。
...
- **示例问题**：必填，固定 3 条输入行；用于帮助用户理解该连接器能做什么。区块标题右侧展示一枚【AI 生成】按钮（一次生成全部 3 条），使用常规字重不使用加粗样式，点击后调用模型根据 MCP 名称和服务描述自动生成示例问题，填入输入行；重复点击可重新生成，覆盖当前内容。单条示例问题最多 300 字符。
- **MCP code**：作为系统内部唯一标识，由系统在首次登记保存时自动生成；登记、编辑和查看状态均不展示，也不允许管理员填写或修改。
- 编辑页不展示 server 自报 ID，也不提供启用或停用控件；发布、撤回和停用统一在列表页操作。

#### 3.1 传输方式切换

- 选择“streamable-http”或“sse”后，展示服务地址和鉴权相关内容，不展示 Command、Arguments、Environment。
- 选择“stdio”后，展示 Command、Arguments、Environment，不展示服务地址和鉴权相关内容。
- 在“stdio”与另外两种之间切换时，清空另一种传输方式已填写的内容，避免两类内容混用。
- 在“streamable-http”与“sse”之间互切时，两者的服务地址和鉴权内容相同，已填写的内容原样保留，不清空。

### 4. 连接与鉴权

- **传输方式**：必填，作为本区块第一项展示；可选择“stdio”、“streamable-http”或“sse”，新建时默认选择“streamable-http”。“sse”是 MCP 旧版远程传输方式（HTTP+SSE，已被 streamable-http 取代），仅为兼容只提供 SSE 端点的服务而保留；选中后传输方式旁提示“旧版 HTTP+SSE；服务方支持 streamable-http 时建议优先使用”。
...
- **MCP 服务地址（Endpoint）**：必填，最多 500 个字符，占位提示为“如 https://example.com/mcp（内网）”；需以 http:// 或 https:// 开头。
- **鉴权方式**：必选，可选择“无鉴权、Bearer Token、API Key”；新建时默认选择“无鉴权”。
...
- **Command**：必填，**下拉选择**，可选 npx / uvx / node / python3 / docker 五项，默认占位提示“npx”；仅命令名称，不含参数（参数填在 Arguments）。不支持自由输入。
- **Arguments**：选填，多行输入，每行填写一个参数，页面展示填写示例。
- **Environment**：选填，以表格形式配置，每行包含变量名、描述、填写方式和平台值四列，页面展示填写示例。
- **描述**：选填，最多 200 个字符，说明该变量的用途；勾选"客户端填写"后该描述会展示给终端用户。
- **填写方式**：勾选"客户端填写"后，平台不保存该变量的值，改由终端用户在客户端自行填写（适用于每人一份的私密凭证）；未勾选时由管理员在"平台值"列填写并由平台保存。
...
- 旧版 HTTP+SSE 传输方式，字段、必填规则和鉴权（无鉴权、Bearer Token、API Key）与 4.1 streamable-http 完全一致，编辑、脱敏、切换鉴权方式等行为同 4.1，不再重复。
- 唯一区别是 **MCP 服务地址（Endpoint）** 的占位提示为“如 https://example.com/sse（内网）”，应填写服务方提供的 SSE 端点。
- 选中“sse”后，传输方式旁展示提示“旧版 HTTP+SSE；服务方支持 streamable-http 时建议优先使用”。
...
- **拉取即落库**：拉取工具针对该连接器已保存的连接配置发起一次真实外呼，同步 server 当前状态，
  结果（工具清单、协议版本、Server 版本、最近探测时间等）随拉取成功立即写回该连接器并生效，不随
  抽屉内其它字段改动的保存或取消而回滚——与【测试连接】"仅在当前抽屉内展示、不改变列表验证状态"
  是两回事（2026-09-23 待办 yuepu#7⑧ 负责人拍板维持现状，补充记录）。
