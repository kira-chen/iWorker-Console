...
- **搜索框**：占位提示为”搜索模型名称或模型标识”，支持按模型名称或模型标识搜索。
...
模型列表用于集中查看和管理平台已接入的模型。列表支持分页，每页条数按窗口高度动态计算，与平台其余列表页采用同一套分页策略。

- **模型名称**：列表只展示管理员设置的模型名称，不展示模型标识；默认模型同时展示【默认】标签。
- **状态**：独立列展示发布状态标签【未发布】【审核中】或【已发布】，紧跟模型名称列之后。
...
- **app_id**：必填，最多 200 个字符，占位提示为“应用归属标识（不参与 HTTP 调用）”。
...
- 「连接与鉴权」区内固定展示【⌁ 验证连通性】按钮，接入和编辑状态下可点击，查看状态置灰不可点。
- 【重新验证】仅在编辑状态展示于底部操作区；接入和查看状态不展示；点击后使用当前已保存的模型配置进行验证，不保存页面中尚未提交的修改。
...
- 各列表与详情页的空值统一展示全角破折号“—”，岗位、专家、技能、模型、MCP、API、业务系统保持一致。
- 提交审核与撤回审核后刷新最近更新时间；重新验证、停用或设置默认模型时，不改变最近更新时间。
- 时间信息均为只读内容，不允许管理员修改。
