...
- **技能分类筛选**：默认"全部技能分类"，可选择"AI Agent、IT 运维与安全、办公效率、行业专业、教育学习、开发编程、内容创作、商业运营、设计多媒体、数据分析、知识管理"。
...
技能列表集中展示平台全部技能。列表支持分页，每页条数按窗口高度动态计算，与平台其余列表页采用同一套分页策略。

- **技能名**：展示图标和技能名称；名称超长换行完整展示。
- **状态**：作为独立列紧跟名称列之后展示，标签为【未发布】【审核中】【已发布】。
- **技能描述**：单行缩略，悬停查看完整内容；无描述显示"—"。
- **技能类型**：展示"岗位私有、市场技能、通用技能"之一，普通文本样式。
- **技能分类**：展示 11 个固定分类之一；未设置显示"—"。
- **工具数**：展示当前引用的工具数量。
...
- 在发布弹窗确认提交后进入审核：技能状态变为"审核中"，提示"已提交发布 vX.Y.Z，进入审核"（与 §四.2 同一句，2026-09-12 负责人决策 2：统一取带版本号的措辞）。
...
- 弹窗标题"新建技能"，内容包括：
  ...
  - **技能分类**：必选 11 个固定分类。
  - **上传方式**：点击【新建技能】后默认进入"上传技能包"方式，初始状态仅展示技能类型和技能包上传区，不展示技能分类。支持单次选择或拖入多个 .zip 技能包（仅支持 .zip、每个须含 SKILL.md；包内是否含 SKILL.md 由服务端解压后校验，前端演示版不解包，仅校验 .zip 后缀）。技能包选中后，在每个技能包文件名后分别出现一个"技能分类"下拉框（11 个固定分类），每个包必须独立选择分类，不能用一个分类批量覆盖全部技能包；支持不同技能包配置不同分类。所有技能包均选择分类后【导入技能包】才可执行，任一技能包未选分类时保留弹窗并提示补齐。导入完成后统一返回技能列表，不自动进入编辑页。每次重新点击【新建技能】时，清空上一次未提交的技能包和分类选择，从初始上传状态开始。或切换"手动创建空白技能"填写技能名（必填，最多 64 字符），点击【创建】后直接跳转至编辑页。
...
- **同名校验（2026-09-18 负责人拍板）**：平台全部技能不可重名，校验对象是 `SKILL.md` 内的 `name` 字段（技能包作者定义的技术名，与下方"技能名称"手动配置的展示名各管各）。zip 导入时逐包校验：命中平台已有技能（含审核状态、含同一批次内先导入的包）时，该包不予导入，行内提示"当前已有同名技能：XXX"（XXX 取该包 `SKILL.md` 内的 `name`）；其余无冲突的包不受影响，按各自结果继续导入。手动创建空白技能时 `SKILL.md` 正文为空，不参与本项校验。
...
- **技能名称**：必填，最多 64 字符，可编辑状态下内嵌输入框；锁定状态纯文本展示，空值显示"未命名技能"。该名称为后台手动配置的展示名，**暂不做重名校验**（2026-09-18 负责人拍板：尽量由配置者自行控制不重名，便于用户端区分不同技能；与"同名校验"一节校验的 `SKILL.md` 内 `name` 字段是两回事）。
- **技能类别标签**：只读展示"操作类 / 查询类"，由工具引用自动派生（引用业务系统等写类工具为操作类，否则为查询类），不可手动修改，悬停可查看派生规则。
- **技能分类**：可编辑状态下必选 11 个固定分类。
...
- **保存**：点击后保存名称、描述、示例问题、技能分类、图标、默认安装及未自动保存的 SKILL.md 正文；成功提示"技能配置已保存"。保存前先校验必填项（表单层拦截，2026-09-21 负责人拍板图标也必填）：技能名称（≤64 字符）、技能分类、图标、描述（≤2000 字符）、示例问题（≤300 字符），任一未通过则提示补齐并不保存；业务系统专属技能没有分类、图标、描述、示例问题行，只校验技能名称。**保存只保存当前配置，不生成版本快照**——版本快照仅在发布审核通过后生成（见"四、版本管理弹窗"）；保存仅刷新"最近更新时间"，不改变技能状态，也不触发审核。
- **效果测试**：预留能力，入口由 feature flag 统一门控，本版默认隐藏（不影响保存/发布流程）。
...
- **示例问题**：必填，1 条，最多 300 字符；占位提示"填 1 个终端用户会问的问题"；用于向终端用户展示提问示例。示例问题输入框旁展示【AI 生成】按钮（使用常规字重，不使用加粗样式），点击后调用模型根据技能名称和描述自动生成示例问题，填入输入框；重复点击可重新生成，覆盖当前内容。
