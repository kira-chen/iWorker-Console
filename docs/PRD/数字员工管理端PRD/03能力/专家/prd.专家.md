...
- **专家帮你做（示例问题）**：必填，固定 3 条输入行，每条最多 60 字符；用于向终端用户展示专家能做什么，如"帮我生成一份行业调研报告"。**区块标题右侧展示一枚【AI 生成】按钮**（常规字重，不使用加粗样式），点击后调用模型根据专家名称和简介一次生成全部 3 条并填入输入行；重复点击可重新生成，覆盖当前内容。专家简介为空时按钮置灰，悬停提示"请先填写专家简介"。
