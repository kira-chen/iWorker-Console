...
- **所属岗位**：仅当专家类型为"岗位私有"时展示，**必填**，**多选**下拉，至少选择 1 个；未选择时提示"请选择所属岗位"。可选择已发布的岗位进行绑定，**一个专家可同时绑定多个不同岗位**。绑定后该专家仅在所选岗位下可用。专家类型创建后不可更改，所属岗位在新建后仍可编辑增减（审核中配置锁定期间不可改），但不能全部移除，至少保留 1 个岗位。
- **图标**：必填。
...
- 点击【创建专家】或【保存】时校验：专家名、分类、图标、简介、职责描述、示例问题必填（专家类型为"岗位私有"时，所属岗位也必填），至少引用 1 个市场技能；示例问题未全填时单独提示"请填写 3 条"专家帮你做"示例问题"并聚焦到首个空输入框；其余未通过时对应区域标红并提示"请先补齐必填项"或"请选择专家分类"。
