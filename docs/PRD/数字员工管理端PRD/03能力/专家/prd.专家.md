...
导航栏用于帮助管理员搜索专家、筛选专家类型、分类和状态，并进入新建专家流程。

- **页面标题**：展示"专家"。
- **页面说明**：文案"平台全部专家 —— 通用专家 / 岗位私有 / 市场专家 统一管理"。
- **搜索框**：占位提示为"搜索专家名、描述或分类"，支持按专家名、简介或分类搜索。
- **专家类型筛选**：默认"全部专家类型"，可选择"岗位私有、市场专家、通用专家"。
...
- 搜索、类型、分类和状态筛选可组合使用；点击【查询】或在搜索框按回车后按当前条件刷新。
- 清空搜索内容后按剩余条件刷新。
- 切换类型、分类或状态后列表立即刷新。
- 点击【＋ 新建专家】保留列表当前筛选条件。
...
专家列表集中展示平台已创建的专家。列表支持分页，每页条数按窗口高度动态计算，与平台其余列表页采用同一套分页策略。

- **专家名**：展示图标和专家名称；图标底色取该专家配置的背景色。
- **状态**：独立列展示发布状态标签【未发布】【审核中】或【已发布】，紧跟专家名列之后。
- **专家描述**：展示专家简介，超长缩略，悬停查看完整内容。
- **专家类型**：展示"岗位私有、市场专家、通用专家"之一，普通文本样式。
- **分类**：展示专家所属分类（通用、法律、财税等 8 项之一）。
- **技能数**：展示当前引用的市场技能数量。
- **引用情况**：岗位私有有引用时展示"N 个岗位引用"（N 为该专家绑定的岗位数），市场专家展示其引用的市场技能数"N 个技能"（无引用技能展示"暂无技能"；2026-09-24 负责人裁定：暂无"应用"引用方的数据来源，按引用技能数展示）；岗位私有点击弹出引用清单（展示岗位名）；无引用展示"暂无引用"；通用专家显示"—"。
...
- **专家名**：必填，最多 64 字符；占位"如 经营分析专家"。未填写时提示"请填写专家名"。**同名校验（2026-09-18 负责人拍板）**：专家名为管理端手动输入，平台全部专家（不分岗位私有 / 市场专家 / 通用专家）不可重名；新建或编辑保存时校验，命中已有专家时该字段标红提示"专家名已存在"，不提交。
- **专家类型**：必选单选"岗位私有 / 市场专家 / 通用专家"，下方提示"专家类型创建后不可更改"。新建时必须选择，编辑时置灰不可修改。
- **所属岗位**：仅当专家类型为"岗位私有"时展示，**多选**下拉。可选择已发布的岗位进行绑定，**一个专家可同时绑定多个不同岗位**；未选择任何岗位时显示"未绑定岗位"（不强制必选）。绑定后该专家仅在所选岗位下可用。专家类型创建后不可更改，所属岗位可在编辑时增减（审核中配置锁定期间不可改）。
- **图标**：必填。
...
- **背景色**：支持从指定色值中选择背景色（#FAE9DF；#DCECF7；#DCF5E4；#E7E4F7；#F7E6F2；#F7EFCD；#DDF0EF）。查看态背景色选择器置灰不可点击；所选颜色用于列表页专家图标底色和客户端卡片背景。
...
- **专家帮你做（示例问题）**：必填，固定 3 条输入行，每条最多 300 字符；用于向终端用户展示专家能做什么，如"帮我生成一份行业调研报告"。**区块标题右侧展示一枚【AI 生成】按钮**（常规字重，不使用加粗样式），点击后调用模型根据专家名称和简介一次生成全部 3 条并填入输入行；重复点击可重新生成，覆盖当前内容。专家简介为空时按钮置灰，悬停提示"请先填写专家简介"。
...
- 展示当前专家可见范围内的知识库：企业级知识库对所有专家可见；类型为「专家」且可见范围包含当前专家的知识库也在此展示。
- 顶部展示搜索框，按知识库名称搜索。
- **字段**：知识库名称、数据源、文档数量、状态、操作。
- 未搜索时默认收起只展示前 2 个，展示【展开更多（N）】（N = 总行数 - 2，与搜索过滤无关）查看全部；搜索时展示全部匹配结果。
- 行内操作：【查看】关闭专家抽屉并跳转至知识库模块打开该知识库查看页面；【检索测试】不关闭专家抽屉，在抽屉之上叠加检索测试弹窗，测试完成后可回到原处继续编辑。
- 无匹配结果时展示"没有匹配的知识库"。
- 专家知识库的发布/停用状态与专家版本发布互相独立，走知识库自身的提交发布/停用审核流程（见《prd.知识库.md》§三.9），不随专家发布联动，也不计入专家发布校验。
...
- 点击【创建专家】或【保存】时校验：专家类型、专家名、分类、图标、简介、职责描述、示例问题必填（专家类型未选提示"请选择专家类型"），至少引用 1 个市场技能；示例问题未全填时单独提示"请填写 3 条"专家帮你做"示例问题"并聚焦到首个空输入框；其余未通过时对应区域标红并提示"请先补齐必填项"或"请选择专家分类"。
- 点击【保存】只保存当前配置，不生成版本快照，不修改列表中的最新版本号，也不自动提交审核。
- 创建成功提示"专家已创建"；保存成功提示"专家配置已保存"；关闭抽屉并刷新列表。
- 编辑状态下底部展示【发布】按钮，点击后自动保存当前编辑页的表单内容（不提示），然后打开版本管理侧栏，与列表页【发布】使用同一流程；若技能引用为空，单独提示"至少引用 1 个市场技能才能发布"，不保存不打开侧栏；新建状态下不展示【发布】。
