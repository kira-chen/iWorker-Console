...
- **搜索框**：占位提示为“搜索用户名、显示名或邮箱”，支持按用户名、显示名或邮箱搜索。
- **角色筛选**：默认展示“全部用户分类”，选项展示当前可选的角色名称。
...
- **搜索或筛选无结果**：展示“没有符合条件的用户”，保留当前查询条件。
- **尚无用户（无任何筛选条件）**：展示“还没有用户 · 点「＋ 新建用户」创建第一个”。
...
- 窗口展示“为 用户显示名（用户名）设置角色。保存后将全量替换当前角色。”；显示名为空时展示用户名。
- 当前角色默认处于选中状态。
- 管理员可选择一个或多个角色。
...
- 【更多】按钮右侧展示展开指示箭头，收起时为 ▾、展开时为 ▴。
- 点击【更多】后，向下展开【重置密码】和【删除用户】；【删除用户】悬停时提示“删除前需二次确认”。
...
- 确认内容为“删除「用户显示名」后，其角色、登录会话、个人文档、任务、记忆及凭证将被永久删除，且无法恢复。”；显示名为空时使用用户名。
- 确认按钮显示【删除】，并使用危险操作样式。
- 管理员取消或关闭确认窗口时，不执行删除，也不展示失败提示。
- 删除成功后提示“用户已删除”并刷新列表。
- 若当前页仅有一条记录且当前页不是第 1 页，删除成功后返回上一页。
- 平台至少保留一个系统管理员，删除最后一个系统管理员时拒绝并提示“不能删除最后一个系统管理员”。
...
- **搜索或筛选无结果**：展示“没有符合条件的用户”（与“暂无用户”区分，便于用户判断是条件过窄还是尚未建号）。
...
- 两种状态底部均展示【取消】和确认按钮；确认按钮文案：新建状态为【新建】，编辑状态为【保存】。
- 保存期间，确认按钮显示进行中状态，不允许重复保存。
- 每次打开窗口时，重新展示当前操作对应的内容，并清除上一次的校验提示。

### 2. 新建用户

- **用户名**：必填，提示文字为“3–32 个字符”；长度为 3～32 位；用户名在平台内唯一。
- **显示名**：必填，提示文字为“用于展示的姓名”。
- **邮箱（选填）**：选填，提示文字为“name@example.com”。
- **初始角色**：必填，支持选择一个或多个角色，提示文字为“选择一个或多个角色”；打开窗口时默认选中“普通用户”，可修改。
- **初始密码提示**：展示“初始密码为 wemate123，用户首次登录后可修改。”

![用户新建页](./用户新建页.png)

### 3. 新建校验

- 用户名为空、或少于 3 位或超过 32 位时，提示“请输入 3–32 个字符”。
- 当前页面仅校验用户名是否填写及长度，不限制字符类型。
- 用户名与平台内已有用户重复时（不区分大小写，如 Zhangwei 与 zhangwei 视为重复），保存失败并提示“用户名已存在”。
- 显示名为空时提示“请输入显示名”。
- 邮箱格式不正确时提示“请输入有效邮箱”。
- 未选择角色时提示“请至少选择一个角色”。
...
- 新建成功后提示“用户已新建”，编辑成功后提示“用户信息已保存”，关闭窗口，用户列表回到第 1 页并刷新。
