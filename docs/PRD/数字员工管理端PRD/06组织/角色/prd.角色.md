...
角色列表用于集中查看角色、绑定用户数及已开通的页面权限。列表支持分页，每页条数按窗口高度动态计算，与平台其余列表页采用同一套分页策略（客户端分页：全量拉取后在前端按页切片，分页条与其他列表页一致）。
...
- 所有角色均展示【删除】，不因角色来源或名称隐藏按钮；【删除】按钮悬停时提示"删除前需二次确认"。
