...
- **背景色**：支持从指定色值中选择背景色（#FAE9DF；#DCECF7；#DCF5E4；#E7E4F7；#F7E6F2；#F7EFCD；#DDF0EF）。背景色选择器仅在新建 / 编辑态展示，查看态不展示；所选颜色用于列表页专家图标底色和客户端卡片背景。
...
- 展示当前专家可引用的知识库，仅限类型为「专家」的知识库。
- 顶部展示搜索框，按知识库名称搜索。
- **字段**：知识库名称、数据源、文档数量、状态、操作。
- 未搜索时默认收起只展示前 2 个，展示【展开更多（N）】查看全部；搜索时展示全部匹配结果。
- 行内操作：【查看】打开知识库查看抽屉（只读）；【检索测试】打开检索测试弹窗。
- 无匹配结果时展示"没有匹配的知识库"。
